...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query(value = "SELECT t.* FROM TRANSACTION_TBL t JOIN (SELECT accountId, MAX(dtTimestamp) as maxTimestamp FROM " +
            "TRANSACTION_TBL t1 JOIN db_sandbox.ACCOUNT_TBL a ON t1.accountId = a.id WHERE a.customerId = :customerId  GROUP BY t1.accountId) tj ON t.accountId = tj.accountId AND t.dtTimestamp = tj.maxTimestamp"
            , nativeQuery = true)
